...
Article images live in `src/assets/images/` and use Markdown image syntax with `@/assets/` paths so Astro optimizes them. Files offered for download live in `public/content/` and use `/content/` links. Legacy static asset redirects are intentionally not retained. The site retains the original KD favicon and, without upstream's demo social image, uses AstroPaper's generated `/og.png`.
